def make_model(data: dict) -> pm.Model:
    ...
    with pm.Model() as model:
        ...
        # Spline coefficients - create as vector but handle Ks=1 case
        if Ks == 1:
            bs_scalar = pm.Flat("bs")
            bs = pt.stack([bs_scalar])
        else:
        ...
        # Spline coefficients - create as vector but handle Ks_sigma=1 case
        if Ks_sigma == 1:
            bs_sigma_scalar = pm.Flat("bs_sigma")
            bs_sigma = pt.stack([bs_sigma_scalar])
        else:
        ...
        mu_linear = Intercept + pt.dot(Xs, bs) + pt.dot(Zs_1_1, s_1_1)
        sigma_linear = Intercept_sigma + pt.dot(Xs_sigma, bs_sigma) + pt.dot(Zs_sigma_1_1, s_sigma_1_1)
