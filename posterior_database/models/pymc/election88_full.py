def make_model(data: dict) -> pm.Model:
    ...
    with pm.Model() as model:
        ...
        age = np.array(data['age']) - 1
        age_edu = np.array(data['age_edu']) - 1  
        edu = np.array(data['edu']) - 1
        region_full = np.array(data['region_full']) - 1
        ...
        # Parameters with bounded uniform priors (no explicit prior in Stan model block)
